...
ls_sparseOLD <- function(bn, bs, ix, iy, nobs, nvars, x, y, pf, dfmax, 
    ...
    gamma <- gamma/nobs
    gamma <- as.double(gamma)
    fit <- .Fortran("ls_f_sparseOLD", bn, bs, ix, iy, gamma, nobs, nvars, as.double(x), 
...
ls_sparseSR <- function(bn, bs, ix, iy, nobs, nvars, x, y, pf, dfmax, 
                      pmax, nlam, flmin, ulam, eps, maxit, vnames, group, intr, asparse, standardize) {
    #################################################################################
    # call Fortran core
    intercept = 0L
    ...
    gamma <- gamma/nobs
    gamma <- as.double(gamma)
    fit <- .Fortran("ls_f_sparseSR", bn, bs, ix, iy, gamma, nobs, nvars, as.double(x), 
                    as.double(y), pf, dfmax, pmax, nlam, flmin, ulam, eps, maxit, intercept, nalam = integer(1), 
                    b0 = double(nlam), beta = double(nvars * nlam), idx = integer(pmax), 
                    nbeta = integer(nlam), alam = double(nlam), npass = integer(1), jerr = integer(1), 
                    alsparse = asparse)
